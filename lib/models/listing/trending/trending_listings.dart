...
class Item {
    ...
    // Vehicle-specific getters from details
    String? get fuelType => details?.vehicles?.fuelType ?? details?.flatDetails?["fuelType"];
    int? get year => details?.vehicles?.year ?? details?.flatDetails?["year"];
    String? get transmission => details?.vehicles?.transmission ?? details?.flatDetails?["transmissionType"];
    int? get mileage => details?.vehicles?.mileage ?? details?.flatDetails?["mileage"];
    String? get make => details?.vehicles?.make ?? details?.flatDetails?["make"];
    String? get model => details?.vehicles?.model ?? details?.flatDetails?["model"];
    
    // Real estate getters from details
    int? get bedrooms => details?.realEstate?.bedrooms ?? details?.flatDetails?["bedrooms"];
    int? get bathrooms => details?.realEstate?.bathrooms ?? details?.flatDetails?["bathrooms"];
    int? get yearBuilt => details?.realEstate?.yearBuilt ?? details?.flatDetails?["yearBuilt"];
    String? get size => details?.realEstate?.size ?? details?.flatDetails?["size"];
    int? get floors => details?.realEstate?.floors ?? details?.flatDetails?["floors"];
    String? get utilities => details?.realEstate?.utilities ?? details?.flatDetails?["utilities"];
    int? get parkingSpaces => details?.realEstate?.parkingSpaces ?? details?.flatDetails?["parkingSpaces"];
    
    // Common getters
    String? get color => details?.vehicles?.color ?? details?.realEstate?.color ?? details?.flatDetails?["color"];
    String? get condition => details?.vehicles?.condition ?? details?.realEstate?.condition ?? details?.flatDetails?["condition"];
...
class Details {
    ...
    factory Details.fromJson(Map<String, dynamic> json) {
        return Details(
            ...
            realEstate: json["real_estate"] != null ? RealEstateDetails.fromJson(json["real_estate"]) : null,
            flatDetails: json["vehicles"] == null && json["real_estate"] == null ? json : null,
        );
    }
